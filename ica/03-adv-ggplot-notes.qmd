---
title: "Visualizing SFO Weather Data"
---
...
NYT wants readers to see that a majority of cities were hotter in 2015 than in any other year. 
...
month_boundaries= weather%>%
  group_by(Month)%>%
  summarize(month_boundary = min(dateInYear))
...
ggplot(weather, aes(x=dateInYear)) +
  ...
  geom_linerange(aes(ymin=Low, ymax=High), color = "#A90248")+
  geom_vline(
  xintercept = month_boundaries$month_boundary[-1],
  linetype = "dashed",
  color = "gray"
)+
  scale_x_discrete(limits=c(1,365))+
  theme_classic()
```
...
month_boundaries = month_boundaries %>%
  mutate(
    ...
    midpoint = (month_boundary + lead(month_boundary, default = 366)) / 2
  )
...
ggplot(weather, aes(x=dateInYear)) +
  ...
  geom_text(
    ...
    aes(x = midpoint, y = -Inf, label = month_name),
    vjust = -1, size = 2
  ) +
  labs(x=NULL)+
  theme_classic()
