...
```{r}
#Remove 99999 values in PrecipYr 
weather$PrecipYr = weather$PrecipYr %>%
  na_if(99999)
```

```{r}
#Create DateInYear
weather$DateInYear = yday(as.Date(weather$date, format = "%m/%d/%y"))
```

```{r}
#Month name variable
weather$month_name = month(as.Date(weather$date, format = "%m/%d/%y"), label=TRUE, abbr=TRUE)
```

```{r}
#Save 
write.csv(weather, "./data/processed/weather_clean.csv")
```
