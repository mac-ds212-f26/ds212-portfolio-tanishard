---
title: "Visualizing MN Spatial Data"
---
...
#I had to run this, comment out if unnecessary
setwd("/Users/tanisha/Intermed. DS/Portfolio/ica/04-adv-maps/code")
...
transform_and_plot <- function(spatial_obj, new_crs, map_title) {
    spatial_obj |> 
        st_transform(crs = new_crs) |> 
        ggplot() +
            geom_sf()+
            labs(title = sprintf("CRS System: %s", map_title))
}

# Example usage of this function (using a South Carolina CRS)
# Modify the call when you modify the function above
transform_and_plot(nc, new_crs = "EPSG:2318", map_title = "EPSG:2318")

# Write your answer to the challenge below
transform_and_plot(nc, new_crs = "EPSG:22293", map_title = "EPSG:22293")
...
st_crs(mn_cities) #NAD83
st_crs(mn_water) #NAD83
...
ggplot() +
    geom_sf(data = mn_counties, alpha = 0.7)+
...
`_d()`: the variable you are scaling by has discrete values. 
`_c()`: the variable you are scaling by has continuous values. 
`_b()`: the variable you are scaling by has binary values.
...
ggplot() +
    ...
    geom_sf(data = mn_cities |> filter(population >= 10000), aes(color = population, size = population))+ # cities layer
    scale_color_viridis_c() + # continuous (gradient) color scale
    labs(title = "Minnesota Cities with Population >= 10,000") + 
    ggthemes::theme_map() +
    theme(legend.position = "bottom") # move legend
...
cities_per_county = cities_per_county%>%
  mutate(name=name.x)
